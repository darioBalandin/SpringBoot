...
public class c5_CreatingSequence {
     ...
     */
    @Test
    public void trouble_maker() throws InstantiationException, IllegalAccessException {
        Mono<String> trouble = Mono.error(IllegalStateException.class.newInstance()); //todo: change this line
    ...
    public void interval() {
        Flux<Long> interval = Flux.interval(Duration.ofMillis(1000l));//todo: change this line only
    ...
    public void repeat() {
        ...
        Flux<Integer> repeated = Mono.fromCallable(counter::getAndIncrement).repeat(10); //todo: change this line
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> createFlux = Flux.create(sink -> {
            for (int i = 0; i <= 5; i++) {
                sink.next(i);//todo: fix following code so it emits values from 0 to 5 and then completes
            }
            sink.complete();
