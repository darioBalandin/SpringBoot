...
//        stonks.subscribe(System.out::println);
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail =
                mailBoxPrimary().switchOnFirst((s, m) -> {
                    if (s.get().metaData.contains("spam")) {
                        System.out.println("esto es: " + s);
...
//        Flux.combineLatest(carChassisProducer(),carEngineProducer(),Car::new);
//                .zipWith(carEngineProducer(), Car::new);
